...
module.exports = class DappTransactions {
	...
	static ballot_initializeProposals(imports) {
	...
	static ballot_issueBallot(imports) {
		return fcl.transaction`
				${DappTransactions.injectImports(imports)}
				transaction {
	...
	static spend_points(imports) {
		return fcl.transaction`
				...
				// SIGNED BY: CUSTOMER
				${DappTransactions.injectImports(imports)}
				transaction(retailerAddrParam: Int, booleanParam: Bool, otherRetailerParam: String) {
